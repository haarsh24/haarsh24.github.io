...
export default () => {
  ...
  const onFinish = (data) => {
    ...
    fetch(Config.contactFormUrl, { method: 'POST', body: formData })
      .then(() => {
        message.success('Thank you for your kind response 🙂. Will get back to you soon.');
        form.resetFields();
      })
  ...
  return (
    <Col sm={24} md={24} lg={12} className="widthFull">
      <Form form={form} name="nest-messages" onFinish={onFinish} validateMessages={validateMessages}>
        ...
        <Form.Item>
          <Button type="primary" shape="round" size="large" htmlType="submit" style={{ backgroundColor: '#C651CD',color:'#100F10' }}>
            SUBMIT
          </Button>
        </Form.Item>
      </Form>
